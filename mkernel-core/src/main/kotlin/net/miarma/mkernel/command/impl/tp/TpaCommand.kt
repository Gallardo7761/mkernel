...
class TpaCommand @Inject constructor(
    ...
    override fun register() {
        commandAPICommand(configService.getString(ConfigKeys.Commands.Tpa.NAME)) {
            ...
            withUsage(configService.getString(ConfigKeys.Commands.Tpa.USAGE))
            playerExecutor { sender, args ->
                ...
                plugin.launchSync {
                    ...
                    if (request != null) {
                        ...
                        messageService.builder(configService.getString(ConfigKeys.Commands.Tpa.MSG_TO)).withPrefix().tag("target", target.name).send(sender)
                        messageService.builder(configService.getString(ConfigKeys.Commands.Tpa.MSG_FROM)).withPrefix().forPlayer(target).tag("sender", sender.name).send(target)
                    }
                }
            }
        }
